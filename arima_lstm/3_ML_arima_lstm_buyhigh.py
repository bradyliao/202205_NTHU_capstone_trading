...
def get_arima(data, train_len, test_len):
    ...
    with open('model_hybrid_arima.pkl', 'wb') as pkl:
...
def get_lstm(data, train_len, test_len, lstm_len=60):
    # prepare train and test data
    data = data.tail(test_len + train_len).reset_index(drop=True)
    ...
    for i in range(lstm_len, train_len):
        x_train.append(dataset[i - lstm_len:i, 0])
        y_train.append(dataset[i, 0])
    for i in range(train_len, len(dataset)):
        x_test.append(dataset[i - lstm_len:i, 0])
    ...
    # Set up & fit LSTM RNN
    model = Sequential()
    model.add(LSTM(units = int(lstm_len), return_sequences = True, input_shape = (x_train.shape[1], x_train.shape[2])))
    ...
    # wtf
    # model.add(LSTM(units = lstm_len, input_shape = (x_train.shape[1], x_train.shape[2])))
    # # model.add(LSTM(units=int(lstm_len/2)))
    # # model.add(LSTM(units=int(lstm_len/4)))
    ...
    # print(prediction)
    prediction = yscaler.inverse_transform(prediction).tolist()
...
if __name__ == '__main__':
    ...
    data = pd.read_csv('Input_0530_buyhigh.csv', index_col=0, header=0).reset_index(drop=True)
    
    num_of_data = len(data)
    train_len = int(num_of_data * 0.9)
    test_len = num_of_data - train_len
    ...
    for i in range(4, 100):
        ...
        for ma in talib_moving_averages:
            # Run moving average, remove last test_len days (used later for test data set), trim MA result to last 60 days
            ma_output = functions[ma](data[:-test_len], i).tail(60)
    ...
    for ma in optimized_period:
        ...
        optimized_period[ma] = int(str(optimized_period[ma]))
        low_vol = functions[ma](data, optimized_period[ma])
        low_vol.fillna(1, inplace=True)
        high_vol = data['high'] - low_vol

        # Generate ARIMA and LSTM predictions
        print('\nWorking on ' + ma + ' predictions')
        try:
            low_vol_prediction, low_vol_mse, low_vol_rmse, low_vol_mape = get_arima(low_vol, train_len, test_len)
        # except:
        ...
        high_vol_prediction, high_vol_mse, high_vol_rmse, high_vol_mape = get_lstm(high_vol, train_len, test_len)

        final_prediction = pd.Series(low_vol_prediction) + pd.Series(high_vol_prediction)
        mse = mean_squared_error(final_prediction.values, data['high'].tail(test_len).values)
        rmse = mse ** 0.5
        mape = mean_absolute_percentage_error(data['high'].tail(test_len).reset_index(drop=True), final_prediction)

        # Generate prediction accuracy
        actual = data['high'].tail(test_len).values
        result_1 = []
        result_2 = []
        for i in range(1, len(final_prediction)):
            # Compare prediction to previous high price
        ...
        simulation[ma] = {'low_vol': {'prediction': low_vol_prediction, 'mse': low_vol_mse,
                          ...
                          'final': {'prediction': final_prediction.values.tolist(), 'mse': mse,
                                    'rmse': rmse, 'mape': mape},
                          'accuracy': {'prediction vs high': accuracy_1, 'prediction vs prediction': accuracy_2}}
    ...
    for ma in simulation.keys():
        print('\n' + ma)
        print('Prediction vs high:\t\t' + str(round(100*simulation[ma]['accuracy']['prediction vs high'], 2))
              + '% Accuracy')
